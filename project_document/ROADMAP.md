...
- 已支持渠道 inbound 适配器，外部渠道消息通过 env 密钥、HMAC-SHA256 签名和 timestamp 时间窗校验后进入统一会话链路。
- 下一步将渠道密钥扩展到数据库配置，并补真实渠道回调协议差异。

## V3: 生产化扩展

- 接入向量数据库和 rerank，替换 V1 关键词检索。
- 真实渠道接入生产化：Web、App、微信、抖音等渠道密钥轮换、回调协议差异和 replay nonce 保护。
