...
- `POST /api/channels/inbound`: 外部渠道消息入口，要求 `channel`、`externalConversationId`、RFC3339 `timestamp` 和 HMAC-SHA256 `signature`；校验会读取 `ChannelIntegration` 的 enabled、secretRef、签名时间窗和 replay 开关，重复签名载荷返回 `409 duplicate_inbound`。
