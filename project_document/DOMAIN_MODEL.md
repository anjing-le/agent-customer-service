...
| `CompleteChannelRunbookCheck` / `BlockChannelRunbookCheck` / `RecoverChannelRunbookCheck` / `ListChannelRunbookChecks` | 确认、阻塞、恢复 Runbook 检查项，并按渠道、Runbook 状态、检查状态、操作者或 action ref 查询处置记录；未完成且 dueAt 已过期的检查项会进入 overdue 汇总 |
