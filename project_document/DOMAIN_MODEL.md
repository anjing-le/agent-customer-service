...
| `ChannelAdapterRequest` | 真实渠道回调的薄适配请求，例如 WeChat/App/Marketplace 字段映射 | `/api/channels/*/inbound` |
...
| `ChannelIntegration` | 渠道接入配置的非敏感视图，记录 active/next secret ref、allowed origins、签名窗口和 replay 开关 | seed store 或 PostgreSQL |
...
| 渠道接入 | adapter 先把真实渠道字段归一为标准 inbound，再读取 `ChannelIntegration` 做来源、HMAC-SHA256 签名、时间窗、enabled、external message id 和 replay 校验 |
