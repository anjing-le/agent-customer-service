...
| `ChannelAdapterRequest` | 真实渠道回调的薄适配请求，例如 WeChat/App/Marketplace 字段映射 | `/api/channels/*/inbound` 与 `contracts/channel-protocol-matrix.json` |
...
| `RuleApproval` | 规则发布审批记录，包含审批人、样本 ID 明细、自动样本数、风险等级和审批状态 | `/api/ops/rules/approve` |
...
| `ChannelNotification` | 渠道告警出站模拟事件，支持运营确认和备注 | `Dashboard.channelNotifications` |
...
| 渠道观测 | Dashboard 聚合失败类型、小时趋势、通知策略和通知事件，用于解释拒绝边界如何进入运营监控 |
...
| 规则发布 | canary 需要先通过带样本明细的 `RuleApproval` 门禁，才能发布为 active；active 可回滚并记录 `RuleReleaseEvent` |
...
- 渠道 adapter 已覆盖 WeChat/App/Marketplace 的示例字段归一；密钥轮换支持 active/next secret ref 双密钥窗口，来源白名单支持 `Origin` / `X-Channel-Origin`。
