...
| `ChannelNotification` | 渠道告警出站通知事件，记录 HMAC 签名、发送次数、重试状态、死信原因和运营确认 | `Dashboard.channelNotifications` |
...
| 渠道观测 | Dashboard 聚合失败类型、小时趋势、通知策略和通知事件；通知事件可演示签名出站、失败重试、死信和运营确认 |
