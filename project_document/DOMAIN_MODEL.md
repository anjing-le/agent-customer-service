...
| 渠道接入 | 对 inbound 请求读取 `ChannelIntegration`，再做 HMAC-SHA256 签名、时间窗、enabled、external message id 和 replay 校验 |
...
- 渠道 replay 保护已覆盖重复签名载荷和真实渠道 `externalMessageId`；下一步可补不同渠道的回调字段适配和密钥轮换流程。
