...
| 场景配置 | Runtime connected | 启用 Intent 参与关键词识别；启用 SYSTEM Prompt 注入 LLM 上下文；启用 Rule 由轻量 RuleEngine 执行 |
...
| RuleEngine | Lightweight runtime | 支持 `SENSITIVE_FILTER`、`TRANSFER_THRESHOLD`、`VIP_PRIORITY` 内置规则码，返回命中原因和动作 |
| 可靠性可观测 | In progress | 对话响应已返回回复引擎、兜底原因、护栏标签和命中规则；前端 Agent 工作区已展示可靠性 Tab |
...
- Scene 已轻量接入运行时，复杂条件表达式和规则命中统计下一阶段继续补。
