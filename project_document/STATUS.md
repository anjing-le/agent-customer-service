...
| 场景配置 | Runtime connected | 启用 Intent 参与关键词识别；启用 SYSTEM Prompt 由 PromptRuntime 渲染后注入 LLM 上下文；启用 Rule 由轻量 RuleEngine 执行 |
...
- Scene 已轻量接入运行时，复杂条件表达式、Prompt 变量 schema 和规则命中统计下一阶段继续补。
