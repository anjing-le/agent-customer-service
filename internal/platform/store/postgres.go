...
func (s *PostgresStore) DispatchChannelNotification(id, outcome string) (ChannelNotification, error) {
	...
	if _, err := s.pool.Exec(context.Background(), `
		...
		set status = $2,
		    ...
		    receipt_body = $13,
		    delivery_audit = $14,
		    dead_letter_reason = $15
		where id = $1
	`, item.ID, item.Status, item.Attempts, item.MaxAttempts, item.BackoffSeconds, nullableTime(item.NextRetryAt, nextRetryAt), item.TargetURL, item.SecretRef, item.Signature, lastDispatchAt, item.LastError, item.ReceiptStatus, item.ReceiptBody, notificationDeliveryAuditJSON(item.DeliveryAudit), item.DeadLetterReason); err != nil {
...
func (s *PostgresStore) UpdateChannelAlertPolicy(channel string, targetURL string, secretRef string, maxAttempts int, backoffSeconds int, actor string, note string) (ChannelAlertPolicy, error) {
	...
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return ChannelAlertPolicy{}, fmt.Errorf("begin update channel alert policy: %w", err)
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return ChannelAlertPolicy{}, fmt.Errorf("commit update channel alert policy: %w", err)
	}
	alerts, err := s.listChannelAlerts()
	if err != nil {
		return ChannelAlertPolicy{}, err
...
func (s *PostgresStore) ApproveNotificationPolicyChange(id string, approver string, note string, confirmation string) (ChannelAlertPolicy, error) {
	...
	if err := expireNotificationPolicyChangesTx(ctx, tx, time.Now().UTC()); err != nil {
		return ChannelAlertPolicy{}, err
	}
...
func (s *PostgresStore) CancelNotificationPolicyChange(id string, actor string, note string) (NotificationPolicyChange, error) {
	ctx := context.Background()
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return NotificationPolicyChange{}, fmt.Errorf("begin cancel notification policy change: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	...
	now := time.Now().UTC()
	canceledBy := fallback(actor, "ops-a")
	if _, err := tx.Exec(ctx, `
		update notification_policy_changes
		set status = 'CANCELED',
		    approved_by = $2,
		    approved_at = $3
		where id = $1
	`, id, canceledBy, now); err != nil {
		return NotificationPolicyChange{}, fmt.Errorf("cancel notification policy change: %w", err)
	}
	change.Status = "CANCELED"
	change.ApprovedBy = canceledBy
	change.ApprovedAt = now.Format(time.RFC3339)
	event := newNotificationPolicyEvent(change.Channel, "CANCEL", canceledBy, notificationPolicySummary(before), notificationPolicyChangeSummary(change), fallback(note, change.Note), now.Format(time.RFC3339))
	if err := insertNotificationPolicyEvent(ctx, tx, event); err != nil {
		return NotificationPolicyChange{}, err
	}
	if err := tx.Commit(ctx); err != nil {
...
func (s *PostgresStore) channelNotification(id string) (ChannelNotification, error) {
	...
	var deliveryAudit []byte
	if err := s.pool.QueryRow(context.Background(), `
		select id, channel, severity, target, target_url, secret_ref, status, reason, count, attempts, max_attempts, backoff_seconds, next_retry_at, signature, last_dispatch_at, last_error, receipt_status, receipt_body, delivery_audit, dead_letter_reason, created_at, acked_by, ack_note, acked_at
	...
	`, id).Scan(
		...
		&item.Attempts, &item.MaxAttempts, &item.BackoffSeconds, &nextRetryAt, &item.Signature, &lastDispatchAt, &item.LastError, &item.ReceiptStatus, &item.ReceiptBody, &deliveryAudit, &item.DeadLetterReason,
		&createdAt, &item.AckedBy, &item.AckNote, &ackedAt,
	); err != nil {
		return ChannelNotification{}, fmt.Errorf("load channel notification: %w", err)
	}
	item.DeliveryAudit = parseNotificationDeliveryAudit(deliveryAudit)
...
func (s *PostgresStore) Dashboard() (Dashboard, error) {
	...
	policyEvents, err := s.listNotificationPolicyEvents()
	if err != nil {
		return Dashboard{}, err
	}
...
func insertNotificationPolicyEvent(ctx context.Context, tx pgx.Tx, event NotificationPolicyEvent) error {
	createdAt, err := time.Parse(time.RFC3339, event.CreatedAt)
	if err != nil {
		createdAt = time.Now().UTC()
	}
	if _, err := tx.Exec(ctx, `
		insert into notification_policy_events (id, channel, action, actor, before_summary, after_summary, note, created_at)
		values ($1, $2, $3, $4, $5, $6, $7, $8)
	`, event.ID, event.Channel, event.Action, event.Actor, event.Before, event.After, event.Note, createdAt); err != nil {
...
func (s *PostgresStore) expireNotificationPolicyChanges() error {
	ctx := context.Background()
	tx, err := s.pool.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin expire notification policy changes: %w", err)
	}
	defer func() { _ = tx.Rollback(ctx) }()
	if err := expireNotificationPolicyChangesTx(ctx, tx, time.Now().UTC()); err != nil {
		return err
	...
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit expire notification policy changes: %w", err)
	}
	return nil
}

func expireNotificationPolicyChangesTx(ctx context.Context, tx pgx.Tx, now time.Time) error {
	...
	`, now)
	if err != nil {
		return fmt.Errorf("list expired notification policy changes: %w", err)
	}
	ids := make([]string, 0)
	for rows.Next() {
		var id string
	...
	for _, id := range ids {
		...
		`, id, now); err != nil {
			return fmt.Errorf("expire notification policy change: %w", err)
		}
		change.Status = "EXPIRED"
		change.ApprovedBy = "system"
		change.ApprovedAt = now.UTC().Format(time.RFC3339)
		event := newNotificationPolicyEvent(change.Channel, "EXPIRE", "system", notificationPolicySummary(before), notificationPolicyChangeSummary(change), "通知策略变更审批超时自动过期", now.UTC().Format(time.RFC3339))
		if err := insertNotificationPolicyEvent(ctx, tx, event); err != nil {
			return err
		}
	}
...
func (s *PostgresStore) listNotificationPolicyEvents() ([]NotificationPolicyEvent, error) {
	rows, err := s.pool.Query(context.Background(), `
		select id, channel, action, actor, before_summary, after_summary, note, created_at
		from notification_policy_events
		order by created_at desc, id desc
		limit 20
	`)
	if err != nil {
		return nil, fmt.Errorf("list notification policy events: %w", err)
	}
	defer rows.Close()

	events := make([]NotificationPolicyEvent, 0)
	for rows.Next() {
		var item NotificationPolicyEvent
		var createdAt time.Time
		if err := rows.Scan(&item.ID, &item.Channel, &item.Action, &item.Actor, &item.Before, &item.After, &item.Note, &createdAt); err != nil {
			return nil, err
		}
		item.CreatedAt = createdAt.UTC().Format(time.RFC3339)
		events = append(events, item)
...
func notificationDeliveryAuditJSON(audit []NotificationDeliveryAudit) string {
	if len(audit) == 0 {
		return "[]"
	}
...
func parseNotificationDeliveryAudit(data []byte) []NotificationDeliveryAudit {
	if len(data) == 0 {
		return nil
	}
	var audit []NotificationDeliveryAudit
