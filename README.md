...
渠道 webhook 会读取 `ChannelIntegration`，执行来源白名单、签名、时间窗、启用状态、`externalMessageId` 对账和重复提交校验。控制台只展示 active/next secret ref 与 allowed origins，不返回密钥值。签名密钥默认使用 demo 值；演示真实渠道时可通过 env 覆盖：
